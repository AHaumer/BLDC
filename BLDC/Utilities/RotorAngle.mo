...
block RotorAngle "Detects rotor angle from hall signals"
...
equation
  y=arg(Modelica.ComplexMath.sum({if uC[k] then Modelica.ComplexMath.exp(j*orientation[k]) else Complex(0) for k in 1:m}));
